...
import com.bldj.lexiang.utils.MD5Utils;
import com.bldj.lexiang.utils.ToastUtils;
import com.bldj.lexiang.utils.Util;
...
//		setContentView(R.layout.pay);
		...
		if(isPaySupported){//检查是否支持微信支付
			new GetAccessTokenTask().execute();
		}else{
			ToastUtils.showToast(this, "您的微信版本太低。");
		}
	}
	...
	private class GetAccessTokenTask extends AsyncTask<Void, Void, GetAccessTokenResult> {
		...
		protected void onPreExecute() {
			dialog = ProgressDialog.show(WXPayActivity.this, getString(R.string.app_tip), getString(R.string.getting_access_token));
		}
		...
		protected void onPostExecute(GetAccessTokenResult result) {
			...
			if (result.localRetCode == LocalRetCode.ERR_OK) {
				Toast.makeText(WXPayActivity.this, R.string.get_access_token_succ, Toast.LENGTH_LONG).show();
				Log.d(TAG, "onPostExecute, accessToken = " + result.accessToken);
				
				GetPrepayIdTask getPrepayId = new GetPrepayIdTask(result.accessToken);
				getPrepayId.execute();
			} else {
				Toast.makeText(WXPayActivity.this, getString(R.string.get_access_token_fail, result.localRetCode.name()), Toast.LENGTH_LONG).show();
			}
		}
		...
		protected GetAccessTokenResult doInBackground(Void... params) {
			...
			String url = String.format("https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%s&secret=%s",
					ApiConstants.APP_ID, APP_SECRET);
	...
	private class GetPrepayIdTask extends AsyncTask<Void, Void, GetPrepayIdResult> {
		...
		protected void onPostExecute(GetPrepayIdResult result) {
			...
			if (result.localRetCode == LocalRetCode.ERR_OK) {
				Toast.makeText(WXPayActivity.this, R.string.get_prepayid_succ, Toast.LENGTH_LONG).show();
				sendPayReq(result);
			} else {
				Toast.makeText(WXPayActivity.this, getString(R.string.get_prepayid_fail, result.localRetCode.name()), Toast.LENGTH_LONG).show();
			}
		}
	...
	private static class GetAccessTokenResult {
		...
		private static final String TAG = "MicroMsg.SDKSample.WXPayActivity.GetAccessTokenResult";
	...
	private static class GetPrepayIdResult {
		...
		private static final String TAG = "MicroMsg.SDKSample.WXPayActivity.GetPrepayIdResult";
